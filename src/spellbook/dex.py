...
VENUE ALLOWLIST + EXECUTION (SPEC §10 v2): the user picks which venues
may execute swaps via ``dex.allowed_venues`` in spellbook.json
(default: matcha + uniswap; ``normalize_venue`` maps the "0x" alias to
"matcha"). Nothing in this module signs or broadcasts. Quotes are
fetched and normalized here; at execution time the daemon fetches the
firm quote again and validates it field-by-field against the
human-approved bounds (``validate_swap_intent_against_quote``) before
anything is signed. A venue outside the user's allowlist is refused at
intent time, and a venue that doesn't serve the intent's chain is
refused too — both fail closed.
...
# ---------------------------------------------------------------------------
# Venue registry — the user-chosen allowlist
# ---------------------------------------------------------------------------
...
#: Allowlist used when spellbook.json names no dex.allowed_venues.
DEFAULT_ALLOWED_VENUES = (VENUE_MATCHA, VENUE_UNISWAP)
...
__all__ = [
    ...
    "VENUE_CHAINS", "VENUE_ENV_KEYS", "DEFAULT_ALLOWED_VENUES",
