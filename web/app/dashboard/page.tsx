    ...
    return (
      <div className="wrap">
        ...
        <header className="dash-hero">
          <span className="kicker">Demo — not real authentication</span>
          <h1>
            Spellbook <span className="dash-wordmark">dashboard</span>
          </h1>
          <p className="lede">
            Prototype login gate. Neither path below performs real
            authentication — they open the read-only demo dashboard so the
            layout can be reviewed. Real challenge-response login needs a
            backend nonce/verify endpoint that does not exist yet.
          </p>
        </header>
        ...
        <div className="dash-panel">
          <div className="dash-grid2">
            <div className="dash-card">
              <span className="dash-step">Path A · Demo</span>
              <span className="icon">🧙</span>
              <h3>Muse / agent challenge-sign</h3>
              <p>
                Simulates the agent login: the server issues a short-lived
                challenge, the agent signs locally with its identity key,
                the server verifies against the expected public key. No
                keys, no signatures happen in this demo — the button just
                opens the dashboard.
              </p>
              <button
                className="btn"
                type="button"
                onClick={() => setLoginPath("agent")}
              >
                Enter as agent (demo) →
              </button>
            </div>
            <div className="dash-card">
              <span className="dash-step">Path B · Demo</span>
              <span className="icon">👁️</span>
              <h3>Human read-only viewer token</h3>
              <p>
                Simulates a human viewer token: watch-only access to
                balances, queue, and activity. This page can never approve,
                sign, or broadcast — read-only is the whole point.
              </p>
              <button
                className="btn ghost"
                type="button"
                onClick={() => setLoginPath("human")}
              >
                Enter as viewer (demo) →
              </button>
            </div>
          </div>
      ...
      <nav className="nav dash-nav">
        ...
        <div className="nav-links">
          ...
          <span className="dash-session">
            {loginPath === "agent" ? "🧙 agent demo" : "👁️ viewer demo"}
          </span>
        </div>
      </nav>
      ...
      {tab === "portfolio" && (
        <section>
          <div className="dash-panel">
            ...
            <p className="dash-note">
              Prototype · testnet data · strictly read-only. This page cannot
